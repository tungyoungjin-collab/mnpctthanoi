...
// ========================================
// TẠO DỮ LIỆU BIỂU ĐỒ: HIỂN THỊ GAPS KHI THIẾU DỮ LIỆU GIỜ TRÒN
// DÙNG item.time TỪ API (đã là Vietnam time UTC+7)
// ========================================
...
function createChartData(data, history, days) {
    ...
    // *** QUAN TRỌNG: LỌC CHỈ LẤY GIỜ TRÒN (:00) ***
    ...
    // Tạo Map từ item.time → waterLevel (để tra cứu nhanh)
    const timeToValueMap = new Map();
    history24h.forEach(item => {
        timeToValueMap.set(item.time, item.waterLevel);
    });
 
    // Lấy thời gian bắt đầu và kết thúc từ dữ liệu (item.time)
    // Ví dụ: "10:00", "11:00", "14:00", ...
    const times = history24h.map(item => item.time);
 
    if (times.length === 0) {
        return { labels: [], values: [], history24h: [], latestTime, startTime };
    }
 
    // Lấy ngày từ record đầu tiên và cuối cùng (dùng item.timestamp để xác định ngày)
    const firstRecord = history24h[0];
    const lastRecord = history24h[history24h.length - 1];
 
    const firstDate = new Date(firstRecord.timestamp);
    const lastDate = new Date(lastRecord.timestamp);
 
    // Tính số giờ giữa first và last record
    const hoursDiff = Math.floor((lastDate.getTime() - firstDate.getTime()) / (60 * 60 * 1000));
 
    // Tạo timeline hoàn chỉnh từ first đến last (mỗi giờ 1 mốc)
    const allHours = [];
    const allLabels = [];
    const allValues = [];
 
    for (let i = 0; i <= hoursDiff; i++) {
        const currentTime = new Date(firstDate.getTime() + i * 60 * 60 * 1000);
        const hours = String(currentTime.getHours()).padStart(2, "0");
        const minutes = String(currentTime.getMinutes()).padStart(2, "0");
        const timeString = `${hours}:${minutes}`;
 
        // Lấy date info để hiển thị trên nhãn
        const day = String(currentTime.getDate()).padStart(2, "0");
        const month = String(currentTime.getMonth() + 1).padStart(2, "0");
        const label = `${day}/${month} ${timeString}`;
 
        allLabels.push(label);
 
        // Tìm giá trị từ map (nếu không có → null → sẽ tạo gap)
        const value = timeToValueMap.has(timeString) ? timeToValueMap.get(timeString) : null;
        allValues.push(value);
    }
 
    return { labels: allLabels, values: allValues, history24h, latestTime, startTime };
}
